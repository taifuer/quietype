...
后台“照片”用于维护 `/photos/` 年度图库，每条记录只需要标题、HTTPS 图片地址和拍摄月份，也可以补充地点、简短说明、尺寸、焦距、光圈、快门、ISO、相机与镜头。页面按拍摄年份和月份倒序排列，横幅、竖幅和方图依据已记录比例组成克制的编辑式网格；照片说明和拍摄参数不挤占列表，只在 PhotoSwipe 放大查看时出现。照片不提供独立详情页或可编辑固定链接，后台“在图库中查看”直接定位到 `/photos/` 中的对应记录；原始图片地址仍是禁用 JavaScript 时的渐进式后备入口。
