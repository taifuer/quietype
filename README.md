...
后台“阅读”使用 WordPress 原生内容类型保存个人书目，分类与标签均由管理员创建。每条记录包含标题、作者、出版社、出版年份、ISBN、阅读月份、阅读状态、豆瓣评分、个人整星评价和一段简短点评；封面使用特色图。公开总览位于 `/books/`，按阅读年份分组、月份倒序呈现；旧 `/reading/` 地址会永久跳转到新书架。阅读状态为已读、在读或待读，并在评分左侧以纯文字显示。标题和封面直接链接到豆瓣，书籍不另设本站详情页，详细阅读总结仍适合写成普通博客文章。
