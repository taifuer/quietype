...
后台“阅读”使用 WordPress 原生内容类型保存个人书目，分类与标签均由管理员创建。每条记录包含标题、作者、出版社、出版年份、ISBN、阅读日期、豆瓣评分、个人整星评价和一段简短点评；封面使用特色图。公开总览位于 `/reading/`，按阅读年份分组、日期倒序呈现。标题和封面直接链接到豆瓣，详细阅读总结仍适合写成普通博客文章。

书籍编辑页接受豆瓣读书链接或条目 ID。点击“读取豆瓣资料”只生成标题、作者、出版社、出版年份、ISBN、评分和封面的预览，不会修改现有字段；核对后还需要点击独立的“确认填入”按钮。保存时可将豆瓣封面经过来源、格式和大小校验后导入媒体库。豆瓣页面结构或访问策略变化时，所有字段仍可手工填写。
